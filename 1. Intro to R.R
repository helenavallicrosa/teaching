...
#### 1. Import data ####
# data format
NP <- read.csv("~/Desktop/NPdata.csv") # change the route between " " according to your files
...
setwd() # change it to a new one

#### 2. Play with data ####
#mention find and replace tool
#subset
library(plyr)
load("~/Desktop/Guaiana Papers/Treball/Above.R")
summary(cars) #R has its data frames charges in the base by default. Cars is one of them
...
cars$mpg
hist(cars$mpg)
ecofriendly <- subset(cars, cars$mpg > 25)
...
# glm
m1 <- glm(yield ~ N+P+K, data = npk) #apply log(N)
...
a <- celsius_to_kelvin(temp_C=e)

#### 3. Good manners and tricks ####
# tabulate according to the subprocess
# same policy with spaces
# search tool
# cmd + alt + c <- convert all selected text to annotation and viceversa
# click on a key. It will highlight in bold the closing one
# you can run a whole section just running its title
...
mixed_m <- function(elem,data){
  ...
  for(i in 1:length(elem)){
    form <- paste("lme(yield ~",elem[i], ", random=~1|block, data=data, na.action=na.omit)")
    m1 <- eval(parse(text=form))
    sum <- summary(m1)
    sum_mods <- data.frame(sum$tTable, sum$AIC, elem[i])
    out <- rbind(out, sum_mods)
  }
  colnames(out)[6:7] <- c("AIC","element")
  return(out)
}
